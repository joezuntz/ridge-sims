...
def plot_shear_all_categories():
    ...
    for cat in CATEGORIES:
        ...
        for run_id, df in shear_list:
            ...
            # NEW: legend label based on mapping file
            legend_label = get_param_label(cat, run_id)

            ax_gplus.plot(arcmin_centers, df[gplus_col], alpha=0.6, label=legend_label)
            ax_gx.plot(arcmin_centers, df[gcross_col], alpha=0.6, label=legend_label)
