...
def make_shared_array(comm, array):
    import numpy as np
    from mpi4py import MPI

    nbytes = array.nbytes if comm.rank == 0 else 0
    win = MPI.Win.Allocate_shared(nbytes, array.dtype.itemsize, comm=comm)
    ptr, _ = win.Shared_query(0)

    # The underlying bytes array
    buffer = np.array(ptr, dtype='B', copy=False)
    # An interface to this of the correct array kind
    output_array = np.ndarray(buffer=buffer, dtype=array.dtype, shape=array.shape)
    save_me = [win, ptr, buffer]
    return save_me, output_array
...
def run_filament_pipeline(run_id):
    ...
    save_me, coords = make_shared_array(COMM_WORLD, coords)
