"""
Test script for interleaved shear calculations
-----------------------------------------------------
Runs shear computation for all runs and bands, alternating between
normal and zero-error directories for each band. This just so We can have 
...
# ======================================================
# === Compute shear for a given ridge directory ===
# ======================================================
def compute_shear_for_ridges(ridges_h5, bg_data, output_dir):
    """Compute filaments, segment them, and compute shear (with and without noise)."""
    ...
    process_shear_sims(filament_h5, bg_data, output_shear_file=shear_flip_csv, flip_g1=True, background_type='sim')
    ...
    np.savetxt(os.path.join(filament_dir, f"shear_p{fp:02d}_shear-randomshear.csv"),
               subtracted_data, delimiter=",",
               header="Bin_Center,Weighted_Real_Distance,Weighted_g_plus_subtracted,Weighted_g_cross_subtracted,Counts,bin_weight", comments="")

    subtracted_data_flip = np.column_stack((shear_data_flip[:, 0], shear_data_flip[:, 1],
                                            g_plus_subtracted_flip, g_cross_subtracted_flip,
                                            shear_data_flip[:, 4], shear_data_flip[:, 5]))
    np.savetxt(os.path.join(filament_dir, f"shear_p{fp:02d}_flipG1_shear-randomshear.csv"),
